...
<Content class="speakers-page" fullscreen="{true}">
					...
					<Col size="12" sizeMd="6">
						<Card class="speaker-card">
							<CardHeader>
								...
								>
									<Avatar toSlot="start">
										<Image alt="{speaker.name + ' profile picture'}" src="{speaker.profilePic}" />
									</Avatar>
									<Label>
										<h2>{speaker.name}</h2>
										<p>{speaker.title}</p>
							...
							<CardContent>
								<List lines="none">
									{#each speaker.sessions as session}
										<Item detail="{false}" href="/app/tabs/speakers/session/{session.id}">
											<Label>
												<h3>{session.name}</h3>
											</Label>
										</Item>

										<Item detail="{false}" href="/app/tabs/speakers/speaker-details/{speaker.id}">
											<Label>
