...
export const NavigationTabGroupData: ComponentDataAPI = {
	description: {
		en: `The NavigationTabGroup is a top level navigation component to implement a tab-based navigation. The component is a container of individual Tab components.

	Both NavigationTabGroup and NavigationTabBar can be used as standalone elements. They don’t depend on each other to work, but they are usually used together in order to implement a tab-based navigation that behaves like a native app.`
	},
	dispatchedEventList: [],
	forwardedEventList: [],
	ionic: {
