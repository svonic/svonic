<script lang="ts">
	...
	import { mdiFormatTextdirectionLToR, mdiFormatTextdirectionRToL } from '@mdi/js';
	import {
		Accordion,
		AccordionGroup,
		Application,
...
<Application class="{darkMode === true ? 'dark' : ''}">
	<NavigationPane class="app-menu" contentId="main-content">
		<Drawer contentId="main-content" id="main-menu">
			<Header>
				<Toolbar>
					<Title>Svonic</Title>
				</Toolbar>
				...
				<List lines="none">
					...
					{#each navPageList as page}
						<DrawerToggle autoHide="{false}">
							<Item detail="{false}" href="{page.route}" svelteKitPrefetch="{true}">
								<Label>{page.label}</Label>
							</Item>
						</DrawerToggle>
					{/each}
					<AccordionGroup multiple="{true}" value="svonic-core">
						<Accordion toggleIconSlot="end" value="svonic-core">
							<Item toSlot="header">
								<Label class="font-bold text-lg">Svonic Core</Label>
							</Item>
							<List toSlot="content">
								{#each navComponentList as componentGroup}
									<AccordionGroup multiple="{true}" value="{selectedComponentGroup}">
										<Accordion toggleIconSlot="end" value="{componentGroup.group}">
											<Item toSlot="header">
			...
			<Header>
				<Toolbar>
					...
					<ButtonGroup toSlot="end">
						<Button href="https://github.com/svonic/svonic" target="_blank">
							<Ionicon icon="{logoGithub}" toSlot="icon-only" />
						</Button>
						<Button on:click="{toogleRTLMode}">
							{#if rtlMode === 'ltr'}
								<Icon path="{mdiFormatTextdirectionRToL}" toSlot="icon-only" />
							{:else}
								<Icon path="{mdiFormatTextdirectionLToR}" toSlot="icon-only" />
							{/if}
						</Button>
						<Button on:click="{toogleDarkMode}">
							{#if darkMode === false}
								<Ionicon icon="{moonOutline}" toSlot="icon-only" />
							{:else}
								<Ionicon color="warning" icon="{sunnyOutline}" toSlot="icon-only" />
							{/if}
						</Button>
					</ButtonGroup>
				</Toolbar>
